...
export class RealtimeAudioServiceV2 extends EventEmitter {
  private websocket: WebSocket | null = null;
  private audioRecorder: WebRTCAudioRecorder | NativeAudioRecorder | null = null;
  ...
  private async setupAudioRecorder() {
    try {
      ...
      // Try WebRTC first, fall back to native
      try {
        this.audioRecorder = new WebRTCAudioRecorder();
        await this.audioRecorder.initialize();
        console.log('Using WebRTC audio recorder');
      } catch (error) {
        console.warn('WebRTC recorder failed, falling back to native:', error);
        this.audioRecorder = new NativeAudioRecorder();
        await this.audioRecorder.initialize();
        console.log('Using native audio recorder');
      }

      // Set up audio recorder event listeners
      this.audioRecorder.on('audioData', (audioData: AudioData) => {
  ...
  private sendAudioData(audioData: AudioData) {
