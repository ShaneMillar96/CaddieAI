...
export class DynamicCaddieService implements DynamicCaddieHelper {
  ...
  private async initializeRealtimeAudio(roundId: number): Promise<void> {
    ...
    try {
      ...
      this.realtimeAudioService.on('responseCompleted', (response: any) => {
        console.log('🏁 DynamicCaddieService: Response completed, processing next in queue');
      ...
      console.log('✅ DynamicCaddieService: Connected to OpenAI real-time audio');
    } catch (error) {
      console.error('❌ DynamicCaddieService: Failed to initialize real-time audio:', error);
      throw error;
    }
  }
  ...
  private addToQueue(request: QueuedRequest): void {
    ...
    console.log(`📋 DynamicCaddieService: Request queued. Queue length: ${this.requestQueue.length}`);
  }
  ...
  private async processNextQueueItem(): Promise<void> {
    console.log(`🔄 DynamicCaddieService: Attempting to process next queue item. Processing: ${this.isProcessingQueue}, Queue length: ${this.requestQueue.length}, Active: ${!!this.activeResponseId}`);
    
    if (this.isProcessingQueue) {
      console.log('⚠️ DynamicCaddieService: Already processing queue, skipping');
      return;
    }
    
    if (this.requestQueue.length === 0) {
      console.log('📋 DynamicCaddieService: Queue is empty');
      return;
    }
    
    if (this.activeResponseId) {
      console.log('⚠️ DynamicCaddieService: Active response in progress, skipping');
      return;
    }
    ...
    if (!request) {
      this.isProcessingQueue = false;
      console.log('❌ DynamicCaddieService: No request found after shifting queue');
      return;
    }

    try {
      console.log(`🔄 DynamicCaddieService: Processing queued request: ${request.scenario} (ID: ${request.id})`);
      this.activeResponseId = request.id;
      
      await this.executeQueuedRequest(request);
      
    } catch (error: any) {
      console.error(`❌ DynamicCaddieService: Error processing queued request ${request.id}:`, error);
    ...
    try {
      ...
      setTimeout(() => {
        if (this.activeResponseId === request.id && this.isProcessingQueue) {
          console.warn('⚠️ DynamicCaddieService: Response timeout, forcing queue continuation');
          this.activeResponseId = null;
          this.isProcessingQueue = false;
          this.processNextQueueItem();
        }
      ...
      console.warn(`🔄 DynamicCaddieService: Using fallback message: "${fallbackMessage}"`);
  ...
  async cleanup(): Promise<void> {
    ...
    if (this.realtimeAudioService && this.isConnected) {
      try {
        ...
        console.log('🧹 DynamicCaddieService: Disconnected from real-time audio service');
      } catch (error) {
        console.error('❌ DynamicCaddieService: Error during cleanup:', error);
      }
    }
